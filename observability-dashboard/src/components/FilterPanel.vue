...
const sessionMap = ref<Map<string, { name: string; timestamp: string }>>(new Map());

const localFilters = ref({ ...props.filters });

const sessionOptions = computed(() => {
  return filterOptions.value.session_ids.map(id => {
    const sessionData = sessionMap.value.get(id);
    return {
      id,
      name: sessionData?.name || `${id.slice(0, 8)}...`
    };
  });
});
...
const fetchFilterOptions = async () => {
  try {
    ...
    // Fetch session names
    const eventsResponse = await fetch('http://localhost:4000/events/recent?limit=1000');
    if (eventsResponse.ok) {
      const data = await eventsResponse.json();
      const events = data.events as HookEvent[];
      
      // Build session map with most recent session_name for each session_id
      const newSessionMap = new Map<string, { name: string; timestamp: string }>();
      events.forEach(event => {
        if (event.session_name && (!newSessionMap.has(event.session_id) || 
            event.timestamp && newSessionMap.get(event.session_id)!.timestamp < event.timestamp.toString())) {
          newSessionMap.set(event.session_id, {
